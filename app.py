        ...
        if (
            ...
            if request.path.startswith("/api/"):
                return jsonify({
                    "success": False,
                    "message": (
                        "Your Librarian session has expired. "
                        "Please verify the security OTP and log in again."
                    ),
                    "login_required": True,
                }), 401

            flash(
                "Please complete Librarian security verification first.",
                "warning",
            )
...
def _issue_librarian_otp(email):
    """
    Create, email and store one secure Librarian OTP.

    Returns: success, wait_seconds, error_message
    """
    ...
    if not secrets.compare_digest(
        ...
        return (
            False,
            0,
            "Enter the approved Librarian email address.",
        )
    ...
    if latest_request is not None:
        elapsed_seconds = _seconds_since(
            latest_request["created_at"]
        )

        if (
            elapsed_seconds is not None
            and elapsed_seconds < OTP_RESEND_SECONDS
        ):
            connection.close()

            return (
                False,
                OTP_RESEND_SECONDS - elapsed_seconds,
                "Please wait before requesting another OTP.",
            )

    otp_upper_limit = 10 ** OTP_LENGTH
    otp_code = str(
        secrets.randbelow(otp_upper_limit)
    ).zfill(OTP_LENGTH)
    ...
    cursor = connection.execute(
        """
        INSERT INTO librarian_otp_requests (
            email,
            otp_hash,
            expires_at,
            attempt_count,
    ...
    send_result = send_librarian_otp(
    ...
    session["pending_librarian_otp_id"] = (
        otp_request_id
    )
    session["pending_librarian_otp_email"] = (
        normalised_email
    )
...
def login():
    """First-stage Librarian security gate using an email OTP."""
    if (
        "username" in session
    ...
    if _librarian_otp_session_is_valid():
        return redirect(
            url_for("librarian_login")
        )
    ...
    if request.method == "POST":
        ...
        success, wait_seconds, error_message = (
            _issue_librarian_otp(email)
        )

        if success:
            flash(
                (
                    "A secure OTP was sent to the "
                    "approved Librarian email."
                ),
                "success",
            )
            return redirect(
        ...
        if wait_seconds > 0:
            ...
            if secrets.compare_digest(
                ...
                flash(
                    (
                        "An OTP was already sent. "
                        f"Please wait {wait_seconds} second(s) "
...
def verify_otp():
    """Verify the emailed OTP before showing the password login."""
    if _librarian_otp_session_is_valid():
        return redirect(
            url_for("librarian_login")
        )

    otp_request_id = session.get(
        "pending_librarian_otp_id"
    )
    pending_email = str(
        session.get(
    ...
    if not otp_request_id or not pending_email:
        flash(
    ...
    otp_request = connection.execute(
        """
        SELECT *
        FROM librarian_otp_requests
        WHERE id = ?
    ...
    if otp_request is None:
        connection.close()
        _clear_librarian_otp_session()
        flash(
            "The OTP request was not found. Request a new OTP.",
            "warning",
        )
        return redirect(url_for("login"))

    expires_at = _parse_local_datetime(
        otp_request["expires_at"]
    )
    now = datetime.now()
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or len(otp_code) != OTP_LENGTH
        ):
            connection.close()
            flash(
                f"Enter the complete {OTP_LENGTH}-digit OTP.",
                "danger",
            )
            return redirect(
                url_for("verify_otp")
        ...
        if otp_request["is_used"]:
            connection.close()
            flash(
                "This OTP has already been used. Request a new OTP.",
                "warning",
            )
            return redirect(url_for("login"))
        ...
        if (
            ...
            flash(
                "The OTP expired. Request a new OTP.",
                "warning",
            )
            return redirect(url_for("login"))

        attempt_count = int(
            otp_request["attempt_count"]
            or 0
        )
        max_attempts = int(
            otp_request["max_attempts"]
            or OTP_MAX_ATTEMPTS
        )
        ...
        if check_password_hash(
            ...
            session[
                ...
                time.time()
                + OTP_VERIFIED_MINUTES * 60
            )

            flash(
                "Email verified. Enter the Librarian credentials.",
                "success",
            )
            return redirect(
                url_for("librarian_login")
    ...
    if otp_request["is_used"]:
        flash(
            "This OTP is no longer active. Request a new OTP.",
            "warning",
        )
        return redirect(url_for("login"))
...
def resend_otp():
    """Send another OTP after the configured cooldown."""
    pending_email = str(
        session.get(
            "pending_librarian_otp_email",
            "",
    ...
    if not pending_email:
        flash(
            "Enter the approved email address again.",
            "warning",
        )
        return redirect(url_for("login"))

    success, wait_seconds, error_message = (
        _issue_librarian_otp(
            pending_email
        )
    )
    ...
    if wait_seconds > 0:
        flash(
            (
                "Please wait "
                f"{wait_seconds} second(s) before resending."
            ),
            "warning",
        )
    else:
...
def reset_security_gate():
    """Return to the email screen and invalidate temporary session data."""
    _clear_librarian_otp_session()
    return redirect(url_for("login"))
...
def librarian_login():
    """Second-stage Librarian username and password login."""
    if (
        "username" in session
        and session.get("role") == "librarian"
    ):
    ...
    if not _librarian_otp_session_is_valid():
        _clear_librarian_otp_session()
        flash(
            "Verify the Librarian email OTP before entering credentials.",
            "warning",
        )
        return redirect(url_for("login"))
